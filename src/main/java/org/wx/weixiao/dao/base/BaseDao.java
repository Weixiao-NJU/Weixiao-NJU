package org.wx.weixiao.dao.base;
...
import org.wx.weixiao.model.FakeDeletable;
...
public class BaseDao<T, ID extends Serializable> implements IBaseDao<T, ID> {
    ...
    @Override
    public void fakeDelete(T t) {
        if (t instanceof FakeDeletable) {
            ((FakeDeletable) t).setIsDelete(1);
            sessionFactory.getCurrentSession().saveOrUpdate(t);
        }
    }

    @Override
    public boolean fakeDeleteById(ID id) {
        T t = get(id);
        if (t == null)
            return false;
        fakeDelete(t);
    ...
    @Override
    public void fakeDeleteAll(Collection<T> entities) {
        for (T entity : entities) {
            this.fakeDelete(entity);
        }
    }
    ...
    public T getByHQL(String hqlString, Object... values) {
        ...
        return fakeGetFilter((T) query.uniqueResult());
    }
...
//        System.out.print(query.getQueryString());
        return fakeListFilter(query.list());
    }
    ...
    private T fakeGetFilter(T object) {
        if(object==null) {
            return null;//可能抛出异常
        }
        if ((object instanceof FakeDeletable) && ((FakeDeletable) object).getIsDelete() == 1)
            return null;
        return object;
    }

    private List<T> fakeListFilter(List<T> li) {
        if (li.size() != 0 && li.get(0) instanceof FakeDeletable) {
            for (int i = 0; i < li.size(); i++) {
                if (li.get(i)!=null&&((FakeDeletable) li.get(i)).getIsDelete() == 1) {
                    li.remove(i);
                    i--;
                }
            }
