$(function(){
	...
	//获取content对象里面，数据的数量
	var number_of_items = $("#answerer-table").find("tr").length-1;

	//计算页面显示的总页数
	var pageCount = Math.ceil(number_of_items/show_per_page);

	//隐藏该对象下面的所有子元素
	$('#answerer-table').find("tr").css('display', 'none');

	//显示第n（show_per_page）元素
	$('#answerer-table').find("tr").slice(0, show_per_page+1).css('display', 'table-row');
	...
	//点击分页按钮触发
	$("#tab1").on("click","#pageGro li",function(){
		...
		$('#answerer-table').find("tr").css('display', 'none').slice(start_point, end_point).css('display', 'table-row');
		$('#answerer-table').find("tr").eq(0).css('display', 'table-row');
	...
	//点击首页
	$("#tab1").on("click","#pageGro .pagestart",function(){
